/* Adapted from data_management/0_run_me_first_data_management.sas (original
   repo file) -- the study's master PROC FORMAT catalog, built once and
   reused by every numbered script in data_management/. The full VALUE
   list below is reproduced verbatim from the source, including the
   mojibake apostrophes/micro signs the author's file already has (Windows
   codepage artifacts, not something this bundle introduces).

   Substitution: the original script reads an external `original` extract
   first (needed by later data-management steps, not by format-building
   itself) and PROC CATALOG COPYs the formats out to a permanent `ss`
   library. Neither step is meaningful in isolation, so this bundle keeps
   only the format-building PROC FORMAT (unmodified) and adds a small
   PROC FORMAT/PROC PRINT smoke test applying a representative sample of
   the ranges -- including the special-missing-value ranges (.D/.E/.M/.P/.Q)
   and an OTHER= catch-all -- against fabricated values, to prove the
   catalog parses and applies correctly. */
...
value packyr_child 		0,3="1) Never smoked before 20 years of age" 1="2) Started before 20, 5 pack-years or less" 2="3) Started before 20, more than 5 pack-years" 4="Unknown status";
value age_start_smok 	0,3="1) Never smoked before 20 years of age" 1,2="2) Before 20 years of age" 4="Unknown smoking status";
...
value bc_min_age 		1,2="2) Started before 20 years of age" 0,3="1) Never used birth control before 20 years of age" 4="Unknown birth control status";
...
/* Smoke test (not in the original file): apply a representative sample of
   the catalog above -- a plain value list (yesno), a numeric range
   (parents_age), the OTHER= catch-all (RSYESNO14_modif), and the
   special-missing-value range (FHMISS2X_modif, .D/.E/.M/.P/.Q) -- against
   fabricated values, to prove the whole catalog parses and applies. */
data check_formats;
	input flag ymflag age_yrs miss_code recode_input;
	format flag yesno. ymflag RSYESNO14_modif. age_yrs parents_age. miss_code FHMISS2X_modif. recode_input FHYESNO_modif.;
	datalines;
0 1 27 .D 1
1 2 33 .M 0
-1 9 22 99 -1
...
proc print data=check_formats noobs;
	title "check_formats (bundle smoke test over the repo's master format catalog)";
run;
