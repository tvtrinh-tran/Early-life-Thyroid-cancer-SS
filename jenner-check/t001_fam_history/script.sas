/* Adapted from data_management/9_fam_history done.sas (original repo file).
   Original reads `original` (LIBNAME'd from an external Sister Study extract)
   and writes to `ss.ss_fam_history`. Here `original` is a small synthetic
   DATA step (10 fabricated participants, no real study data) built with the
   same column names the script keeps, and output goes to WORK so the
   recoding logic below runs completely unmodified. */
...
data want_ss_fam_history;
	set ss_fam_history;
	keep psid fam_history_DTC;
run;

proc print data=want_ss_fam_history noobs;
	title "ss_fam_history (bundle output; original writes to ss.ss_fam_history)";
run;
