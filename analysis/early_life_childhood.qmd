...
univariable_childhood=cox_model(surv = "Surv(ident_ageexact_bl, ident_EOF, ident_DTC)",
...
multivariable_childhood=cox_model(surv = "Surv(ident_ageexact_bl, ident_EOF, ident_DTC)",
                             ...
                             data=earlylife_popu)|>sort_childhood()|>modify_column_hide(columns=c(n_event,exposure))

#assemble descriptive table and cox models' results
cox_childhood = tbl_merge(
  tbls = list(descript_childhood, univariable_childhood, multivariable_childhood),
  tab_spanner = c("**N (%)**",
                  "**Univariable models**",
                  "**Multivariable models**")) |> polish_childhood(group_name = TRUE)
...
mutual=tbl_merge(list(growth_SES_mutual,growth_SES_mutual_all),tab_spanner = c("**Model 1**", "**Model 2**"))|> 
  ...
  modify_footnote(abbreviation = TRUE,
                         `estimate_1` ~ "Model 1 were simultaneously adjusted for height at age 10 vs. peers, and teenage weight vs. peers, ever not having enough to eat during childhood, highest household educational level at age 13, self-identified race/ethnicity, baseline BMI, smoking status, personal history of benign thyroid disease, personal educational level, household annual income, and Area Deprivation Index. Model 2 were simultaneously adjusted for all covariates of model 1 and family income while growing up, household composition at age 13, and childhood residence") 
mutual
```

## Figures 2: Association between height at age 10 vs peers, teenage weight vs peers, ever not having enough to eat during childhood, and highest household education level at age 13 and thyroid cancer incidence in the Sister Study participants stratified according to potential modifying factors
...
ado_weight_adult_bmi=ado_weight_adult_bmi$table_body|>filter(n_event>0)|>
...
child_weight_adult_bmi=child_weight_adult_bmi$table_body|>filter(n_event>1)|> #n_event>1 to exclude Unknown category
  mutate(group=c("Same weight (ref)","Same weight","Same weight",rep("Lighter",3),rep("Heavier",3)))|>
  mutate(group=fct_relevel(group,c("Lighter","Same weight","Same weight (ref)","Heavier")))|>
  mutate(bmi=as_factor(rep(c("BMI <25","BMI 25.0-29.9", "BMI 30.0+"),3)))


results_ado_weight_adult_bmi = ado_weight_adult_bmi|>
...
results_child_weight_adult_bmi=child_weight_adult_bmi|>
  ggplot(aes(x = group,y = estimate, ymin = conf.low, ymax = conf.high ))+
  geom_pointrange(cex=3)+
  geom_hline(aes(fill=bmi),yintercept =1, linetype=2)+
...
## Nitty gritty analyses

## Supplementary table 1: The joint associations of teenage weight vs. peers and baseline BMI with differentiated thyroid cancer incidence and of weight at age 10 vs. peers and baseline BMI with differentiated thyroid cancer incidence

```{r}
#| label: joint-weight-table
###Create lists of exposure and covariates that have a significant association


early_weight_strat_bmi=tbl_merge(list(child_weight_adult_bmi,ado_weight_adult_bmi),
                                 tab_spanner = c("**Weight at age 10**","**Teenage weight vs. peers**"))
early_weight_strat_bmi

multivariable_childhood=cox_model(surv = "Surv(ident_ageexact_bl, ident_EOF, ident_DTC)",
                             var_name = "an_bmi_cont",
                             var_label = list("an_bmi_cont" ~ "BMI"),
                             covar = covariates_childhood,
                             data=earlylife_popu)|>sort_childhood()
ado_weight_bmi_cont=stratification_long(strat=an_weight_teen,
                    surv = "Surv(ident_ageexact_bl, ident_EOF, ident_DTC)",
                    var_name = "an_bmi_cont",
                    var_label = list("an_bmi_cont" ~ "BMI"),
                    covar = covariates_childhood[covariates_childhood!="an_bmi_cat"],
                    #filter out an_bmi_cat because we are working on an_bmi_cont
                    data=earlylife_popu)|>sort_childhood()
age10_weight_bmi_cont=stratification_long(strat=an_weight_age10,
                    surv = "Surv(ident_ageexact_bl, ident_EOF, ident_DTC)",
                    var_name = "an_bmi_cont",
                    var_label = list("an_bmi_cont" ~ "BMI"),
                    covar = covariates_childhood[covariates_childhood!="an_bmi_cat"],
...
#| label: early-onset
#| output: false
#Create datasets for early-onset and late-onset analyses
...
sensi_censor_childhood = tbl_merge(
  tbls = list(cox_early_onset_childhood,cox_late_onset_childhood,cox_cmprsk_childhood, 
