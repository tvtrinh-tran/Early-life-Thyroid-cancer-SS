...
var_label_childhood = list(
  "an_weight_age10" ~ "Relative weight vs. peers at age 10"    ,
  "an_height_age10" ~ "Relative height vs. peers at age 10"    ,
  "an_weight_teen" ~ "Relative weight vs. peers during teen years"      ,
...
mutual=tbl_merge(list(growth_SES_mutual,growth_SES_mutual_all),tab_spanner = c("**Model 1**", "**Model 2**"))|> 
  ...
  modify_footnote(abbreviation = TRUE,
                         `estimate_1` ~ "Model 1 was simultaneously adjusted for attained age (timescale), self-identified race/ethnicity, relative height vs. peers at age 10, and relative weight vs. peers during teen years, ever not having enough to eat during childhood, and highest household educational level at age 13. Model 2 was simultaneously adjusted for all covariates of model 1 and family income while growing up, household composition at age 13, and childhood residence") 
...
results_ado_weight_adult_bmi = results_ado_weight_adult_bmi|>
  ...
  xlab('Relative weight vs. peers during teen years')+ ylab("Hazard Ratio (95% Confidence Interval)")+
...
results_child_weight_adult_bmi=results_child_weight_adult_bmi|>
  ...
  geom_pointrange(cex=3)+
  geom_hline(aes(fill=bmi),yintercept =1, linetype=2)+
  xlab('Relative height vs. peers at age 10')+ ylab("Hazard Ratio (95% Confidence Interval)")+
  geom_errorbar(aes(ymin=conf.low, ymax=conf.high),width=0,cex=1.5)+ 
  #labs(tag = "A") +
  facet_wrap(~bmi,strip.position="bottom",nrow=1,scales = "free_x") +
  theme(plot.title=element_text(size=32,face="bold"),
...
strat_height_age10 = stratification_long_for_a_list(var_name = "an_height_age10",
                                                    var_label = list("an_height_age10" ~ "Relative height vs. peers at age 10"),
                                                    strat_list =  strat_list,
                                                    surv = "Surv(ident_ageexact_bl, ident_EOF, ident_DTC)",
                                                    covar = covariates_childhood,
                                                    data = earlylife_popu) 
png(filename=here::here("childhood_height_age10_strat.png"),pointsize=14,width = 1000, height = 1200)
strat_height_age10 |>forest_plot_child(cat = "Taller",label1 = "Relative height vs. peers at age 10: ",label2 = "Taller vs same height")
dev.off()

strat_weight_teen = stratification_long_for_a_list(var_name = "an_weight_teen",
                                                         var_label = list("an_weight_teen" ~ "Relative weight vs. peers during teen years"),
                                                         strat_list =  strat_list,
                                                         surv = "Surv(ident_ageexact_bl, ident_EOF, ident_DTC)",
                                                         covar = covariates_childhood,
                                                         data = earlylife_popu) 
png(filename=here::here("childhood_weight_teen_lighter_strat.png"),pointsize=14,width = 1000, height = 1200)
strat_weight_teen|>forest_plot_child(cat = "Lighter",label1 = "Relative weight vs. peers during teen years: ",label2 = "Lighter vs same weight")
dev.off()

png(filename=here::here("childhood_weight_teen_heavier_strat.png"),pointsize=14,width = 1000, height = 1200)
strat_weight_teen|>forest_plot_child(cat = "Heavier",label1 = "Relative weight vs. peers during teen years: ",label2 = "Heavier vs same weight")
dev.off()
...
#p-interaction for ado_weight and baseline BMI
ado_weight_adult_bmi_no_interaction=coxph(Surv(ident_ageexact_bl, ident_EOF, ident_DTC)~an_weight_teen+an_bmi_cat+ses_race, 
                                          method = "breslow", data = earlylife_popu)
ado_weight_adult_bmi_with_interaction=coxph(Surv(ident_ageexact_bl, ident_EOF,ident_DTC)~an_weight_teen*an_bmi_cat+ses_race,
                                          method = "breslow", data = earlylife_popu)
anova(ado_weight_adult_bmi_no_interaction,ado_weight_adult_bmi_with_interaction)

#p-interaction for ado_weight and baseline edu levels
ado_weight_adult_edu_no_interaction=coxph(Surv(ident_ageexact_bl, ident_EOF, ident_DTC)~an_weight_teen+ses_edu_level+ses_race, 
                                          method = "breslow", data = earlylife_popu)
ado_weight_adult_edu_with_interaction=coxph(Surv(ident_ageexact_bl, ident_EOF,ident_DTC)~an_weight_teen*ses_edu_level+ses_race,
                                          method = "breslow", data = earlylife_popu)
anova(ado_weight_adult_edu_no_interaction,ado_weight_adult_edu_with_interaction)

#p-interaction for ado_weight and baseline household income
ado_weight_adult_income_no_interaction=coxph(Surv(ident_ageexact_bl, ident_EOF, ident_DTC)~an_weight_teen+ses_income+ses_race, 
                                          method = "breslow", data = earlylife_popu)
ado_weight_adult_income_with_interaction=coxph(Surv(ident_ageexact_bl, ident_EOF,ident_DTC)~an_weight_teen*ses_income+ses_race,
                                          method = "breslow", data = earlylife_popu)
anova(ado_weight_adult_income_no_interaction,ado_weight_adult_income_with_interaction)
